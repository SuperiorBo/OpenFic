...
def create_react_agent(
    ...
    async def tools_join(
        ...
        if is_done:
            ...
            # Break edit/read-retry death spirals after parallel tool fan-out.
            failed_names: list[str] = []
            for outcome in outcomes:
                message = outcome.get("message")
                if isinstance(message, ToolMessage) and _tool_message_indicates_failure(
                    message
                ):
                    failed_names.append(
                        str(outcome.get("tool_name") or getattr(message, "name", "") or "")
                    )
